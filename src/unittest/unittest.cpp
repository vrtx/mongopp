...
namespace unittest {
    ...
    Result * Suite::run( const std::string& filter, int runsPerTest ) {
        ...
        for ( std::vector<TestHolder*>::iterator i=_tests.begin(); i!=_tests.end(); i++ ) {
            ...
            std::cerr << "\t going to run test: " << tc->getName() << std::endl;
        ...
        std::cerr << "\t DONE running tests" << std::endl;
    ...
    int Suite::run( const std::vector<std::string>& suites , const std::string& filter , int runsPerTest ) {
        ...
        if (_allSuites().empty()) {
            std::cerr << "error: no suites registered.";
            return EXIT_FAILURE;
        }

        for ( unsigned int i = 0; i < suites.size(); i++ ) {
            if ( _allSuites().count( suites[i] ) == 0 ) {
                std::cerr << "invalid test suite [" << suites[i] << "], use --list to see valid names" << std::endl;
                return EXIT_FAILURE;
            }
        }
        ...
        for ( std::vector<std::string>::iterator i=torun.begin(); i!=torun.end(); i++ ) {
            ...
            std::cerr << "going to run suite: " << name << std::endl;
        ...
        std::cerr << totals.toString(); // includes endl

        // summary
        if ( !totals._fails.empty() ) {
            std::cerr << "Failing tests:" << std::endl;
            for ( std::vector<std::string>::const_iterator i=totals._fails.begin();
                  i!=totals._fails.end(); i++ ) {
                const std::string& s = (*i);
                std::cerr << "\t " << s << " Failed";
            }
            std::cerr << "FAILURE - " << totals._fails.size() << " tests in "
                  << failedSuites.size() << " suites failed";
        }
        else {
            std::cerr << "SUCCESS - All tests in all suites passed";
        }
        std::cerr << std::endl;
        return rc;
    }
    ...
    TestAssertionFailureException::TestAssertionFailureException(
        ...
        : _details(theFile, theLine, theFailingExpression) {
    }
